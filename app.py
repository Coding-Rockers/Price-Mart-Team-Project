...
class Product(db.Model):
    ...
    def __init__(self, product_name, color, barcode):
...
class StoreProduct(db.Model):
    ...
    def __init__(self, store_product, price):
...
#fix this (Not working as it should)
def top_selling_products(x_days=7):
    x = (
        db.session.query(StoreProduct.price * OrderProduct.quantity).join(Product).join(OrderProduct).join(Order)
        .filter((Order.order_date > (datetime.now() - timedelta(days=x_days))) & Order.store_id == StoreProduct.store_id
                )
    )
    return x
    ...
    def index(self):
        try:
            ...
            return self.render('admin/index.html', store_count=store_count, customer_count=customer_count,
                               ...
                               revenue_past_1_days=revenue_past_x_days(1), top_selling_products=top_selling_products(14),
...
admin = Admin(app, name='Control Panel', template_mode='bootstrap4', index_view=MyHomeView())
...
admin.add_view(NotificationsView(name='Notifications', endpoint='notifications'))
